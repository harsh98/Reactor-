...
public class ReactorController implements Runnable {
	...
	double maxNextTempRodUp = current * Reactor.TEMP_FACTOR
		- this.model.getRodsHeight() + 1 + Reactor.RAND_HIGH;
	double maxNextTempRodDown = current * Reactor.TEMP_FACTOR
		- this.model.getRodsHeight() - 1 + Reactor.RAND_HIGH;
	double minNextTempRodDown = current * Reactor.TEMP_FACTOR
		- this.model.getRodsHeight() - 1 + Reactor.RAND_LOW;
