...
public class Reactor implements Runnable {
     ...
     */
    public enum Status {
	FINISHED("Finished"), MELTDOWN("MELTDOWN!!"), OPERATING(
    ...
    private static final double POWER_FACTOR = MAX_POWER
	    / (MAX_TEMP - MIN_POWER_TEMP);
	...
	final int value = new Random().nextInt(RAND_HIGH - RAND_LOW + 1)
		+ RAND_LOW;
	return value;
    }
     ...
     */
    public Reactor(final double initialTemperature,
	...
	this.power = this.temperature >= MIN_POWER_TEMP
		? (long) ((this.temperature - 100) * POWER_FACTOR)
		: 0;
    ...
    /**
     * Drops the rods entirely into the reactor core - i.e. set the rods lengths
     * to the maximum rods lengths.
     */
    ...
    /**
     * Lower the rod heights by one step. Rods cannot be lowered by more than
     * one step per tick.
     */
    public void lowerRods() {

	if (!this.rodsChanged) {
	    this.rodsHeight = Math.min(Math.max(0, this.rodsHeight + 1),
		    ROD_LENGTH);
	    this.rodsChanged = true;
	}
    }
    ...
    /**
     * Increment the simulation tick by one. Update the reactor temperature,
     * power, and status (in that order), and allow the rods to be raised or
     * lowered during this tick.
     */
    public void tick() {
	...
	this.temperature = this.temperature * TEMP_FACTOR - this.rodsHeight
		+ randomFactor();
